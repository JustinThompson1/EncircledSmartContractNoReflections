  ...
  : describe("Token Unit Tests", function () {
      ...
      describe("Ownable", function () {
        describe("Transfer/Renounce ownership", function () {
          it("Transfer ownership should be possible when called by the owner", async function () {
            await tokenContract.transferOwnership(charles.address);
            expect(await tokenContract.owner()).to.be.equal(charles.address);
          });
          it("Transfer ownership should not be possible when called by a non owner address", async function () {
            await expect(
              tokenContractCharles.transferOwnership(charles.address)
            ).to.be.revertedWith("Ownable: caller is not the owner");
          });
          it("Renouncing ownership should be possible when called by the owner", async function () {
            await tokenContract.renounceOwnership();
            expect(await tokenContract.owner()).to.be.equal(
              "0x0000000000000000000000000000000000000000"
            );
          });
          it("Renouncing ownership should not be possible when called by a non owner address", async function () {
            await expect(
              tokenContractCharles.renounceOwnership()
            ).to.be.revertedWith("Ownable: caller is not the owner");
          });
        });
      });
        ...
        describe("Transaction Fee", function () {
          it("Fee receiving address should get the right amount", async function () {
            ...
            expect(toBalance2).to.be.equal(
              toBalance1.add((transactionFee * transferAmount) / 100)
            );
          });
